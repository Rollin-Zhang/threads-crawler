...
# ( cd Threads && scrapy crawl threads -a scroll_limit=10 -a search_keyword="公館" -a group_number=0 )
import os
import scrapy
...
class ThreadsSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        # cookies 與 storage_state 都位於 threads_scraper 目錄（非 spiders）
        cookies_path = os.path.normpath(os.path.join(project_root, '..', 'cookies.json'))
        # 追加：偵測 storage_state 是否存在（優先使用）
        storage_state_path = os.path.normpath(os.path.join(project_root, '..', 'storage_state.json'))
        ...
        self.logger.info(f"Detect storage_state at: {storage_state_path} exists={os.path.exists(storage_state_path)}")
        self.logger.info(f"Detect cookies at: {cookies_path} exists={os.path.exists(cookies_path)}")

        # 若已有 storage_state，直接以 Playwright 開首頁驗證登入狀態，並跳過帳密登入流程
        if os.path.exists(storage_state_path):
            ThreadsSpider.login_completed = True
            yield scrapy.Request(
                url="https://www.threads.net/?hl=zh-tw",
                callback=self.verify_logged_in,
                headers=headers,
                meta=dict(
                    playwright=True,
            ...
            # 後續如有設定 target 或 search_keyword，照常進行
            yield from self.process_accounts_after_login(headers)
            if self.search_keyword:
                yield from self.process_search_after_login(headers)

        # 若 storage_state 不存在但 cookies 存在，先嘗試轉成 storage_state 再走驗證流程
        elif os.path.exists(cookies_path):
            try:
                self._convert_cookies_to_storage_state(cookies_path, storage_state_path)
                self.logger.info(f"[cookies->storage_state] 已轉換：{cookies_path} -> {storage_state_path}")
            except Exception as e:
                self.logger.warning(f"[cookies->storage_state] 轉換失敗，改走帳密登入：{e}")
                # 直接走登入流程
                yield scrapy.Request(
            ...
            # 無 storage_state 與 cookies，走帳密登入
            yield scrapy.Request(
                url="https://www.threads.net/login/?hl=zh-tw",
                callback=self.login_and_save_cookies,
                headers=headers,
    ...
    async def verify_logged_in(self, response):
        """
        使用 storage_state 啟動後，快速檢查登入狀態是否有效。
        """
        page = response.meta.get("playwright_page")
        try:
            # 先等 DOMContentLoaded，避免 networkidle 長時間等待
            await page.wait_for_load_state("domcontentloaded")

            # 依序嘗試幾種穩定指標（任何一個命中即視為已登入）
            selectors = [
            ...
            if logged_in:
                self.logger.info("[storage_state] 已判定為登入狀態（selector/cookie）。")
            else:
                self.logger.warning("[storage_state] 無法判定登入狀態，可能為未登入或 UI 改變。")

            # 驗證後即刻保存 cookies 與 storage_state，供下次啟動繼承（即便未登入也保存以便診斷）
            project_root = os.path.dirname(os.path.abspath(__file__))
            cookies_path = os.path.normpath(os.path.join(project_root, '..', 'cookies.json'))
            storage_state_path = os.path.normpath(os.path.join(project_root, '..', 'storage_state.json'))
            cookies = await page.context.cookies()
            ...
            self.logger.info(f"[login] 狀態已保存：cookies->{cookies_path}, storage_state->{storage_state_path}")
        except Exception as e:
            self.logger.warning(f"[storage_state] 驗證或保存狀態時發生異常：{e}")
        finally:
            if page:
                await page.close()
