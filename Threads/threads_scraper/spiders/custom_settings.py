...
CUSTOM_SETTINGS_threads = {
    # 使用 Playwright 內建 Chromium（避免依賴系統 Chrome channel）
    ...
    # 下載 handler：僅將 HTTPS 交給 Playwright（避免為 http/https 各啟動一個瀏覽器）
    "DOWNLOAD_HANDLERS": {
...
# 指定 Playwright context 預設載入 storage_state.json（若存在）
STORAGE_STATE_PATH = os.path.normpath(os.path.join(
    os.path.dirname(__file__), "..", "storage_state.json"
))

context_config = {}
if os.path.exists(STORAGE_STATE_PATH):
    context_config["storage_state"] = STORAGE_STATE_PATH

CUSTOM_SETTINGS_threads.update({
    "PLAYWRIGHT_CONTEXTS": {
